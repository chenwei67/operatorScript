...
| Top 慢查询（按 normalized_query 聚合）     | `GROUP BY normalized_query` 聚合 `count/p95/avg/sum(read_bytes)/max(memory_usage)`，按 p95 降序取 TopN                                                                                     | `slow_queries_top.csv`                               | 高       | 找到最该优先优化的查询模式（迁移前治理、迁移后回归验证）             |
...
| 节点磁盘使用明细（VM 瞬时 query）  | `api/v1/query` 拉 `node_filesystem_size/free` 计算 used/used%                         | `snapshots/cluster_node_disk_usage.csv`         | 低（对 CK）/中（对 VM） | 用于容量水位评估，避免磁盘不足导致迁移/回放失败                   |
